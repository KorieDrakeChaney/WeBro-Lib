use std::ops::{Add, Sub, Mul, Div};
use num::*;
use crate::vectors::{Vector2, Vector3};


#[derive(Clone, Copy, Debug, PartialEq)]
pub struct Vector4<T : Num> {
    pub x : T, 
    pub y : T, 
    pub z : T, 
    pub w : T
}

impl<T : Num> Vector4<T> {
    pub fn new(x : T, y : T, z : T, w : T) -> Self {
        Self {
            x, 
            y, 
            z, 
            w
        }
    }   
}
 
impl Default for Vector4<f32> {
    fn default() -> Self {
        Self {
            x : 0.0,
            y : 0.0,
            z : 0.0,
            w : 0.0,
        }
    }
}

impl Default for Vector4<i32> {
    fn default() -> Self {
        Self {
            x : 0,
            y : 0,
            z : 0,
            w : 0,
        }
    }
}


impl<T: Num> Add<Self> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn add(self, other : Self) -> Self{
        Self {
            x : self.x + other.x, 
            y : self.y + other.y,
            z : self.z + other.z, 
            w : self.w + other.w, 
        }
    }
}

impl<T: Num> Sub<Self> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn sub(self, other : Self) -> Self{
        Self {
            x : self.x - other.x, 
            y : self.y - other.y,
            z : self.z - other.z, 
            w : self.w - other.w, 
        }
    }
}

impl<T: Num> Mul<Self> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn mul(self, other : Self) -> Self{
        Self {
            x : self.x * other.x, 
            y : self.y * other.y,
            z : self.z * other.z, 
            w : self.w * other.w, 
        }
    }
}

impl<T: Num> Div<Self> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn div(self, other : Self) -> Self{
        Self {
            x : self.x / other.x, 
            y : self.y / other.y,
            z : self.z / other.z, 
            w : self.w / other.w, 
        }
    }
}



impl<T: Num> Add<Vector3<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn add(self, other : Vector3<T>) -> Self{
        Self {
            x : self.x + other.x, 
            y : self.y + other.y,
            z : self.z + other.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Sub<Vector3<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn sub(self, other : Vector3<T>) -> Self{
        Self {
            x : self.x - other.x, 
            y : self.y - other.y,
            z : self.z - other.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Mul<Vector3<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn mul(self, other : Vector3<T>) -> Self{
        Self {
            x : self.x * other.x, 
            y : self.y * other.y,
            z : self.z * other.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Div<Vector3<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn div(self, other : Vector3<T>) -> Self{
        Self {
            x : self.x / other.x, 
            y : self.y / other.y,
            z : self.z / other.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Add<Vector2<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn add(self, other : Vector2<T>) -> Self{
        Self {
            x : self.x + other.x, 
            y : self.y + other.y,
            z : self.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Sub<Vector2<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn sub(self, other : Vector2<T>) -> Self{
        Self {
            x : self.x - other.x, 
            y : self.y - other.y,
            z : self.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Mul<Vector2<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn mul(self, other : Vector2<T>) -> Self{
        Self {
            x : self.x * other.x, 
            y : self.y * other.y,
            z : self.z, 
            w : self.w, 
        }
    }
}

impl<T: Num> Div<Vector2<T>> for Vector4<T> {
    type Output = Self;
    #[must_use]
    fn div(self, other : Vector2<T>) -> Self{
        Self {
            x : self.x / other.x, 
            y : self.y / other.y,
            z : self.z, 
            w : self.w, 
        }
    }
}